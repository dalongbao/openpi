"""
vla_eval.py — BASE (un-fine-tuned) pi0.5 control run.
Loads the plain-gemma base weights (LoRA variants swapped out) for a base-vs-finetuned
comparison on the same object_in_bowl sim scene.
...
CHECKPOINT_DIR = "/base_weights"   # bind /cluster/work/cvg/data/Egoverse/pi05_base_jax -> /base_weights
...
try:
    ...
    # Base weights are plain gemma (no LoRA adapters). Swap the LoRA variants out so
    # restore_params matches the base checkpoint structure (same as run_inference.py).
